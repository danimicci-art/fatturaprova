...
function checkUser() {
    ...
    if (userProfiles.includes(username)) {
        ...
        // New user - continue to folder selection
        currentUser = username;
        showSetupStep(2); // Go to folder selection step
    }
}
...
function completeSetup() {
    ...
    if (!doctorData.name || !doctorData.address || !doctorData.piva || 
        !doctorData.cf || !doctorData.professional_title || !doctorData.medical_order) {
    ...
    // Try to save to file system first, fallback to localStorage
    saveUserDataToFS().then(success => {
        if (!success) {
            saveUserData(); // Fallback to localStorage
        }
        ...
        showSetupStep(4); // Show final step (was step 3)
    }).catch(error => {
        console.error('Error saving user data:', error);
        // Fallback to localStorage
        saveUserData();
        showNotification('Dati salvati in modalità locale', 'warning');
...
function loadUser(username) {
    ...
    loadUserDataFromFS(username).then(success => {
        if (!success) {
            // Fallback to localStorage
            loadUserDataFromLocalStorage(username);
        }
...
function loadUserDataFromLocalStorage(username) {
    ...
    // Load counters
    const counters = getUserData(username, 'counters') || {
        invoice: 1,
        service: 1
    };
...
function saveUserData() {
    ...
    // Try file system first, then fallback to localStorage
    if (selectedFolderPath) {
        saveUserDataToFS().catch(error => {
            console.error('File system save failed, using localStorage:', error);
...
// Additional setup step functions
...
// Enhanced user menu with folder management
...
function showFolderManagementModal() {
    // Create or show folder management modal
    let modal = document.getElementById('folder-management-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'folder-management-modal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-overlay">
                <div class="modal-content">
                    <h3>Gestione Cartella Dati</h3>

                    <div class="settings-section">
                        <h4>Cartella Attuale</h4>
                        <div class="settings-row">
                            <span class="settings-label">Percorso:</span>
                            <span class="settings-value" id="current-folder-path">${selectedFolderPath || 'Non configurato'}</span>
                            <button onclick="changeFolder()" class="btn btn--sm btn--secondary settings-action">Cambia</button>
                        </div>

                        <div class="settings-row">
                            <span class="settings-label">Modalità:</span>
                            <span class="settings-value">${fileSystemSupported ? 'File System API' : 'Compatibilità'}</span>
                        </div>
                    </div>
                    ...
                    <div class="settings-section">
                        ...
                        <div class="modal-actions">
                            <button onclick="exportUserData()" class="btn btn--secondary">Esporta Backup</button>
                            <button onclick="importUserData()" class="btn btn--secondary">Importa Backup</button>
                            <button onclick="syncData()" class="btn btn--primary">Sincronizza Dati</button>
                        </div>
                    </div>

                    <div class="modal-actions">
                        <button onclick="hideFolderManagementModal()" class="btn btn--outline">Chiudi</button>
                    </div>
                </div>
            </div>
        `;
...
function hideFolderManagementModal() {
...
async function changeFolder() {
    try {
        ...
        if (pathElement) {
            pathElement.textContent = selectedFolderPath || 'Non configurato';
        }

        // Save new path
        saveFolderPath();

        showNotification('Cartella aggiornata', 'success');

    } catch (error) {
        console.error('Error changing folder:', error);
        showNotification('Errore nel cambio cartella', 'error');
    }
}

async function syncData() {
    try {
        showFileSystemStatus('Sincronizzando...', 'loading');

        // Save current data to file system
        await saveUserDataToFS();

        showFileSystemStatus('Sincronizzazione completata', 'success');
        showNotification('Dati sincronizzati', 'success');

    } catch (error) {
        console.error('Error syncing data:', error);
        showFileSystemStatus('Errore sincronizzazione', 'error');
        showNotification('Errore nella sincronizzazione', 'error');
    }
}

// Override existing functions to trigger file system saves
const originalHandleFormSubmit = handleFormSubmit;
function handleFormSubmit(e) {
    // Call original function
    originalHandleFormSubmit(e);

    // Trigger file system save
    if (selectedFolderPath) {
        saveUserDataToFS().catch(error => {
            console.error('Error saving to file system after form submit:', error);
        });
    }
}
